...
const CartProduct = props => {
    ...
    return (
        ...
        <Image className='object-cover h-[80px] w-[60px]' src={productData.image} alt={productData.title} width={60} height={80}></Image>
            <h3 className='w-[200px]'>{productData.title}</h3>
            <div className='flex justify-end items-center space-x-2 group'>
            <form className='flex flex-wrap mr-5'>
            ...
            <button className='pl-2' type='button' onClick={() => cart.deleteFromCart(id)}><AiOutlineCloseCircle className='hover:text-rose-500 text-xl'/></button>
            </div>
            </div>
            </div>
        </div>    
