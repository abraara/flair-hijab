...
const ContactPage = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_4kit2al', 'template_vlofdbi', form.current, 'csAFxIQKXWLwc-pcI')
      .then((result) => {
          ...
          alert("Error occured, message did not send, please try again at a later time.")
      });
      form.current.reset()
  };
  ...
  <div className="py-4 lg:py-8 px-8 md:px-2 mx-auto max-w-screen-md">
      ...
      <form ref={form} onSubmit={sendEmail} class="space-y-8">
          ...
          <button type="submit" name='send' className="py-3 px-5 font-medium text-center rounded-lg bg-pink-300 border border-pink-400 sm:w-fit hover:bg-pink-500 focus:ring-4 focus:outline-none focus:ring-pink-300 hover:text-white">Send Message</button>
      </form>
  </div>
  </main>
  );
